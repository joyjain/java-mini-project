...
/**
 *
 * convert l = new Length(4, 2, 0, 1, 1); l.convert();
 */
public class Length {

    /**
     * "nanometre", "Micrometre", "millimetre", "Centimetre", "Metre",
     * "Kilometre", "Inch", "Foot", "Yard", "Mile"
     */
    ...
    public Length(int from, int to, int type1, int type2, double input) {
        this.from = from;
        this.to = to;
        this.type1 = type1;
        this.type2 = type2;
    ...
    public double convert() {
        if (type1 == type2) {
            ...
            input = imperialToUS(convertImperial(from, 3));
    ...
    private double imperialToUS(double input) {
        return (3.93701 * Math.pow(10, -1) * input);
    }

    private double USToImperial(double input) {
        return (25400000 * input);
    }
}
